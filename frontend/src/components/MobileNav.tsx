...
export function MobileNav({ items, userName, userRole, onLogout }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {/* Overlay + Drawer renderizados via Portal para escapar do stacking context
         do header (sticky z-20) que impedia o drawer de ficar acima do conteúdo */}
      {createPortal(
        <>
          {/* Overlay */}
          {isOpen && (
          ...
          <div
            className={`
              fixed top-0 left-0 z-[60] h-full w-72 bg-white shadow-xl transform transition-transform duration-300 ease-in-out lg:hidden
